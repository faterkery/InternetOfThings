...
function _mediaImg(blankPath, attrs) {
	var width = attrs.width,
		...
		style = '',
		flashvars = attrs.flashvars,
		quality = attrs.quality,
		allowfullscreen=attrs.allowfullscreen,
		loop = attrs.loop,
		pluginspage = attrs.pluginspage;
	if (/\D/.test(width)) {
		style += 'width:' + width + ';';
	} else if (width > 0) {
...
_extend(KCmd, {
	...
	insertimage : function(url, title, width, height, border, align) {//===============jiangtao edit=============================
		title = _undef(title, '');
		border = _undef(border, 0);
		var html = '<img src="' + myroot + _escape(url) + '" data-ke-src="' + myroot + _escape(url) + '" ';
		if (width) {
...
_extend(KUploadButton, {	
	init : function(options) {
		var self = this,
			...
			cls = function(){
				if(_IE && _V < 9) {
					cls = ''/** edit by adrian:button[0].className ||**/ 
				}else{
			...
			var dialog = self.createDialog({
				...
				yesBtn : {
					...
					click : function(e) {
                        ...
                        {							
                        var html = K.mediaImg(self.themesPath + 'common/blank.gif', {
                                        src : contextPath+'/eportalstyle/kindeditor-master/flvplayer.swf',
                                        type : K.mediaType(url),
                                        //flashvars : 'file='+ url +'&autostart='+ (autostartBox[0].checked ? 'true' : 'false') +'',
                                        flashvars : 'vcastr_file='+ url +'&LogoText=www.zsepsp.cn&IsAutoPlay=1',
                                        width : width,
                                        height : height,
                                        quality : 'high',
