...
public abstract class DispatchActions extends DispatchAdminAction {
	...
	public String updateProcess(WebContext context) throws ServletException {
		...
		if (context.getParameter("insert") != null
			...
			//add by jiangtao
			/*Persistent p = getService().getObjectById(getActionClass(), id);
			Searchinfo s = p.getSearchinfo();
			if (s != null) {
				Searchinfo info = getService().getSearchinfo(s.getType(), id);
				if (info != null) {
					info.setTitle(s.getTitle());
					info.setSummary(s.getSummary());
					info.setContent(s.getContent());
					info.setImg(s.getImg());
					getService().updateObject(info);
				}
			}*/
...
//		commonService.delObject(this.getActionClass(), id);
		...
		if (s != null) {
			Searchinfo info = getService().getSearchinfo(s.getType(), id);
			if(info!=null){
				commonService.delObject(Searchinfo.class, info.getId());
			}
		}*/
	...
	public ActionForward showParaDelete(ActionMapping mapping, ActionForm form,
		...
		Object member = WebHelper.getMemberContext(context);
		if (member == null) {
			return mapping.findForward("/admin.html");
		}
		int id = context.getSIntParameter("sid");
		CommonService commonService = getService();
		String re = DELETE_FORWARD;
