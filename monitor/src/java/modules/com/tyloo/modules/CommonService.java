...
import com.tyloo.fw.dao.CommonDao;
import com.tyloo.fw.dao.DAOException;
import com.tyloo.fw.orm.Persistent;
import com.tyloo.fw.waf.BasePOForm;
import com.tyloo.util.BeanUtils;
import com.tyloo.util.searchcontext.SearchCondition;
...
public abstract class CommonService {
	 ...
	 * @param id
	 */
	public void delLogicObject(Class p, int id) {
		Persistent pp = this.getObjectById(p, id);
		pp.setIsDeleted(1);
		dao.update(pp);
	}
	...
	public List<Persistent> getBizObjList(SearchContext searchContext,
		...
		if (StringUtils.isEmpty(where.toString())) {
			query.append(" where isDeleted=0 ");
		} else {
		...
		//add BY JIANGTAO
		String orderBy2 = "desc";
		String orderByField2 = searchContext.getOrderByField2();
		if(StringUtils.isNotEmpty(orderByField2) && StringUtils.isNotEmpty(orderBy2))
		{
			query.append(" ,t.");
			query.append(orderByField2);
			query.append(" ");
			query.append(orderBy2);
		}
